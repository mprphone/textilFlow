...
const STEP_TYPE_LABEL = {
  cutting: 'Corte', sewing: 'Confeção interna', subcontract: 'Subcontrato',
...
function renderTab(tab, data, meta) {
  ...
  if (tab === 'production') return `<div class="card"><div class="card-header"><h2>Gama operatória</h2><button class="btn primary" data-add-operation>+ Operação</button></div>${rowsTable(['Seq.','Operação','Máquina','SMV','Objetivo/h','Qualidade'],data.routing.map(row=>`<tr><td>${row.sequence}</td><td><b>${esc(row.operation_code)}</b> · ${esc(row.operation_name)}</td><td>${esc(row.machine_type||'Manual')}</td><td>${number(row.smv)} min</td><td>${number(row.target_units_hour)}</td><td>${row.quality_checkpoint?badge('checkpoint'):'—'}</td><td><button class="btn small danger" data-delete-resource="product-operations" data-delete-id="${row.id}">Eliminar</button></td></tr>`))}<p class="muted">SMV total: <b>${number(data.routing.reduce((sum,row)=>sum+row.smv,0))} minutos</b></p></div>
    <div class="card section-spaced"><div class="card-header"><h2>Sequência de produção</h2><button class="btn primary" data-add-chain-step>+ Passo</button><button class="btn" data-save-chain>Guardar sequência</button></div><div data-chain-list>${renderChainList(data.production_route || [], data.chain_services)}</div><p class="muted">A ordem define a sequência obrigatória deste artigo — pode misturar corte, confeção interna e subcontratos. Sem nenhum passo aqui, aplica-se a regra geral: corte sempre antes de qualquer subcontrato.</p></div>`;
  ...
  if (tab === 'costs') return `<div class="card"><div class="card-header"><h2>Folhas de custo</h2><button class="btn primary" data-add-sheet>+ Nova versão</button></div>${data.cost_sheets.length ? rowsTable(['Versão','Estado','Materiais','Mão de obra','Máquina','Subcontrato','Indiretos','Custo total','Venda','Margem'],data.cost_sheets.map(row=>`<tr><td>V${row.version}</td><td>${badge(row.status)}</td><td>${money(row.material_cost)}</td><td>${money(row.labor_cost)}</td><td>${money(row.machine_cost)}</td><td>${money(row.subcontract_cost)}</td><td>${money(row.overhead_cost)}</td><td><b>${money(row.total_cost)}</b></td><td>${money(row.selling_price)}</td><td>${number(row.margin_pct)}%</td><td><button class="btn small primary" data-view-sheet="${row.id}">Ver ficha</button> <button class="btn small" data-rebuild-sheet="${row.id}">Recalcular</button></td></tr>`)) : '<p class="muted">Ainda não há nenhuma ficha de custo para este artigo.</p>'}</div>`;
...
async function bindActions(container, data, styleId, back, activeTab, meta) {
  ...
  container.querySelectorAll('[data-view-sheet]').forEach(button=>button.addEventListener('click',async()=>{
    const { renderProposalDetail } = await import('../costing/proposals.js?v=20260827-8');
    await renderProposalDetail(container, Number(button.dataset.viewSheet), () => renderStyleDetail(container, styleId, back, 'costs'));
  }));
