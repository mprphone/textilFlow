...
function drawTask(container, task) {
  ...
  target.innerHTML = `<div class="terminal-grid">
    ...
    <div class="card"><div class="card-header"><h2>Novo registo</h2><span id="timer-value">00:00:00</span></div><div class="actions"><button class="btn success" data-start>▶ Iniciar</button><button class="btn" data-stop>■ Parar</button><button class="btn danger" data-downtime>Registar paragem</button></div><div class="form-grid"><div class="field"><label>Peças boas<input id="floor-good" type="number" min="0" value="1"></label></div><div class="field"><label>Rejeitadas<input id="floor-rejected" type="number" min="0" value="0"></label></div><div class="field"><label>Minutos<input id="floor-minutes" type="number" min="0" step="0.01" value="0"></label></div><div class="field"><label>Energia (€)<input id="floor-energy" type="number" min="0" step="0.01" value="0"></label></div><div class="field"><label>Consumíveis (€)<input id="floor-consumables" type="number" min="0" step="0.01" value="0"></label></div><div class="field"><label>Afinação/setup (€)<input id="floor-setup" type="number" min="0" step="0.01" value="0"></label></div><div class="field full"><label>Observações<textarea id="floor-notes"></textarea></label></div><div class="form-footer"><button class="btn primary" data-submit-output>Confirmar produção</button></div></div></div>
  </div>`;
...
async function submitOutput(container, task) {
  try {
    ...
    await post('/production/events',{assignment_id:task.id,quantity_good:Number(document.getElementById('floor-good').value||0),quantity_rejected:Number(document.getElementById('floor-rejected').value||0),duration_minutes:Number(document.getElementById('floor-minutes').value||0),energy_cost:Number(document.getElementById('floor-energy')?.value||0),consumables_cost:Number(document.getElementById('floor-consumables')?.value||0),setup_cost:Number(document.getElementById('floor-setup')?.value||0),event_type:'output',notes:document.getElementById('floor-notes').value,source:'terminal'});
