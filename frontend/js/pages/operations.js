...
export async function render(container) {
  ...
  await renderEntityPage(container, {
    ...
    fields:[
      ...
      {key:'cost_per_minute',label:'Custo por minuto',type:'number',default:0,section:'Método'},
    ...
    columns:[{key:'code',label:'Código'},{key:'name',label:'Operação'},{key:'department',label:'Área'},{key:'machine_type',label:'Máquina'},{key:'standard_time_min',label:'SMV',render:r=>`${number(r.standard_time_min)} min`},{key:'cost_per_minute',label:'Custo/min',render:r=>money(r.cost_per_minute)},{key:'active',label:'Estado',render:r=>badge(r.active?'ativa':'inativa')}],
  });
}
