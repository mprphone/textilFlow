...
class ProductionRouteStep(Base, TimestampMixin):
    ...
    product_operation_id = Column(Integer, ForeignKey("product_operations.id"), nullable=True, index=True)
    service_stage_id = Column(Integer, ForeignKey("service_stages.id"), nullable=True, index=True)
