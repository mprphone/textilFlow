...
from ..models import ActualCostEntry, ProductionBatch, ProductionOrder, ProductionOrderVariant, QualityInspection, SalesOrderLine, Shipment, ShipmentLine, StyleVariant


EPSILON = 0.001
ACTIVE_SHIPMENT_STATUSES = {"planned", "ready", "shipped"}
...
def _shipped_for_production_order(db: Session, production_order_id: int, *, variant_id=None, by_variant=False) -> float:
    query = (
        ...
        .filter(Shipment.status.in_(ACTIVE_SHIPMENT_STATUSES))
    )
    if by_variant:
...
def _active_shipments(db: Session, sales_order_id: int) -> list[Shipment]:
    return (
        ...
        .filter(Shipment.status.in_(ACTIVE_SHIPMENT_STATUSES))
        .order_by(Shipment.shipped_at.asc(), Shipment.id.asc())
        .all()
    )
...
def dispatch_status(db: Session, sales_order) -> dict:
    ...
    shipped_quantity = round(sum(_quantity(row.quantity) for row in shipments), 2)
    ...
    for line in lines:
        ...
        for order in production_orders:
            ...
            available = max(0.0, round(min(produced, approved, packed) - shipped, 2))
            ...
            if variants:
                ...
                for variant in variants:
                    ...
                    variant_available = min(order_available, max(0.0, _quantity(variant.quantity) - already_variant))
                    if scoped_quality:
                        variant_available = min(variant_available, max(0.0, approved_quantity(db, order, variant.variant_id) - already_variant))
                    if scoped_packing:
                        variant_available = min(variant_available, max(0.0, _packed_for_variant(db, order.id, variant.variant_id) - already_variant))
...
def create_partial_shipment(db: Session, sales_order, payload: dict) -> tuple[Shipment, list[ShipmentLine], dict]:
    ...
    if requested_allocations:
        ...
        for requested_row in requested_allocations:
            key = (int(requested_row.get("production_order_id") or 0), requested_row.get("variant_id"))
            if key in seen:
            ...
            if not candidate or quantity <= EPSILON:
                continue
        ...
        requested = _quantity(payload.get("quantity"))
    ...
    if not shipment.shipment_no or not shipment.carrier:
        raise ValueError("Indique o documento e o transportador")
    db.add(shipment)
    db.flush()
    ...
    for candidate in candidates:
        ...
        db.add(row)
        shipment_lines.append(row)
        remaining = round(remaining - take, 2)
    if remaining > EPSILON:
        raise ValueError("Nao foi possivel alocar toda a quantidade as ordens de fabrico")
    db.flush()

    total_after = round(status["shipped_quantity"] + requested, 2)
    sales_order.status = "shipped" if total_after + EPSILON >= status["ordered_quantity"] else "partially_shipped"
    for row in shipment_lines:
        order = db.get(ProductionOrder, row.production_order_id)
        from .operations_control import allocate_finished_goods
        allocate_finished_goods(db, shipment, row, payload.get("user_id"))
        data = dict(order.custom_data or {})
        recorded = _shipped_for_production_order(db, order.id)
        data["shipped_quantity"] = max(_quantity(data.get("shipped_quantity")), recorded)
        order.custom_data = data
    transport_cost = round(float(payload.get("transport_cost") or 0), 4)
    if transport_cost > 0:
        for row in shipment_lines:
            share = round(transport_cost * float(row.quantity or 0) / max(requested, EPSILON), 4)
            db.add(ActualCostEntry(
                ...
                amount=share, occurred_on=date.today(), reference=f"shipment:{shipment.id}",
                user_id=payload.get("user_id"),
            ))
    db.flush()
    return shipment, shipment_lines, dispatch_status(db, sales_order)
